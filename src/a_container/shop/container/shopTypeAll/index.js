    ...
    render() {
        ...
        return (
            <div className='shop-type-all'>
                <div className={"l-box"}>
                    <div key={0} className={this.state.barId === 0 ? 'check' : ''} onClick={()=>this.barTabChange(0)}>
                        <div>热销产品</div>
                    </div>
                    {
                        d.map((item, index)=>{
                            return (
                                <div key={index} className={this.state.barId === item.id ? 'check' : ''} onClick={()=>this.barTabChange(item.id)}>
                                    <div>{item.name}</div>
                                </div>
                            );
                        })
                ...
                <div className={"r-box"}>
                    {
                        ...
                        tabTags.length ? (
                            <Tabs
                                tabs={tabTags.map((item)=> ({ title: item.tagName, code: item.tagCode }) )}
                                swipeable={false}
                            ...
                            >
                                {(() => {
                                    ...
                                    arr.push(...tabTags.map((v, i) => {
                                        const tData = v.tagCode === 0 ? tabData.productList : tabData.productList.filter((tItem)=>{
                                            const types = tItem.productTag ? tItem.productTag.split(',') : [];
...
export default connect(
    ...
    (dispatch) => ({
        actions: bindActionCreators({ getProDuctList, listProductType, mallApList, pushCarInterface, shopCartCount, getRecommend }, dispatch),
    })
)(HomePageContainer);
