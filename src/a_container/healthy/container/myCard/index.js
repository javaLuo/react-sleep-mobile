          ...
          alert('确认赠送?', '赠送后您的卡将转移给对方，您将无法再查看该卡，该赠送24小时内有效', [
              { text: '取消', onPress: () => console.log('cancel') },
              {
                  ...
                  onPress: () => new Promise((resolve, rej) => {
                      ...
                      const str = `${u.id}_fff_${encodeURIComponent(u.nickName)}_fff_${encodeURIComponent(u.headImg)}_fff_${obj.id}_fff_${obj.cardPrice}_fff_${encodeURIComponent(obj.validTime)}_fff_${dateTime}`;
                      wx.onMenuShareAppMessage({
